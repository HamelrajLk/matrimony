...
const MM_PLAN_ICONS: Record<string, string> = {
  FREE: '🌸', GOLD: '✨', DIAMOND: '💎', PLATINUM: '👑',
};
...
/* ═══════════════════════════════════════════════════════════════════════════
   PAYMENT MODAL (shared between individual & matchmaker upgrade pages)
═══════════════════════════════════════════════════════════════════════════ */
        ...
        <div style={{ padding: '24px 28px 30px' }}>
          ...
          {step === 'choose' && (
            <>
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 700, fontSize: '1rem', color: '#2A1A1A' }}>Pay with Card</div>
                  <div style={{ fontFamily: "'Outfit',sans-serif", fontSize: '0.8rem', color: '#7A6A5A', marginTop: 2 }}>Visa, Mastercard, Amex · Secured by Stripe</div>
                </div>
                <span style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 700, color: '#635BFF', fontSize: '0.85rem', whiteSpace: 'nowrap' }}>Instant →</span>
              </button>
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 700, fontSize: '1rem', color: '#2A1A1A' }}>Bank Transfer (Sri Lanka)</div>
                  <div style={{ fontFamily: "'Outfit',sans-serif", fontSize: '0.8rem', color: '#7A6A5A', marginTop: 2 }}>Direct bank deposit · Activated within 24 hours</div>
                </div>
                <span style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 700, color: '#F4A435', fontSize: '0.85rem', whiteSpace: 'nowrap' }}>Details →</span>
              </button>
          ...
          {step === 'stripe' && (
            <div style={{ textAlign: 'center' }}>
              ...
              >
                {submitting ? 'Redirecting to Stripe…' : `🔒 Pay £${price.priceAmount.toLocaleString()} with Stripe`}
              </button>
            ...
            <div>
              <div style={{ background: '#FFF8F0', border: '1.5px solid #F4A43540', borderRadius: 16, padding: '16px 18px', marginBottom: 20 }}>
                ...
                {([
                  ...
                  <div key={row.key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: '1px dashed #F0E0D0' }}>
              ...
              <form onSubmit={handleBankSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div>
                  ...
                  <input style={INP} placeholder="e.g. Kasun Perera" value={form.senderName}
                    onChange={e => setForm(f => ({ ...f, senderName: e.target.value }))} required />
                </div>
                <div>
                  <label style={{ fontFamily: "'Outfit',sans-serif", fontSize: '0.8rem', fontWeight: 600, color: '#5A4A3A', display: 'block', marginBottom: 5 }}>
                    Your Bank Name
                  </label>
                  <input style={INP} placeholder="e.g. People's Bank, Sampath Bank…" value={form.senderBank}
                ...
                <div>
                  ...
                  <input style={INP} placeholder="e.g. TXN123456789" value={form.transferRef}
                    onChange={e => setForm(f => ({ ...f, transferRef: e.target.value }))} required />
                </div>
                <div>
                  <label style={{ fontFamily: "'Outfit',sans-serif", fontSize: '0.8rem', fontWeight: 600, color: '#5A4A3A', display: 'block', marginBottom: 5 }}>
          ...
          {step === 'bank-done' && (
            <div style={{ textAlign: 'center', padding: '8px 0' }}>
              ...
              <button onClick={onClose} className="btn-primary" style={{ marginTop: 24, padding: '12px 36px' }}>
                Got it, thanks! →
              </button>
            </div>
          )}
...
/* ═══════════════════════════════════════════════════════════════════════════
   MATCHMAKER UPGRADE PAGE
═══════════════════════════════════════════════════════════════════════════ */
export default function MatchmakerUpgradePage() {
  ...
  useEffect(() => {
    ...
    if (payment === 'success' && sessionId) {
      ...
      api.post('/api/payments/stripe/verify-session', { sessionId })
        ...
        .finally(() => router.replace('/partners/dashboard/upgrade'));
    } else if (payment === 'cancelled') {
      toastShown.current = true;
      toast.error('Payment was cancelled.');
      router.replace('/partners/dashboard/upgrade');
    }
  }, [searchParams, router]);

  /* load matchmaker plans */
  useEffect(() => {
    (async () => {
      try {
        const [plansRes, subRes] = await Promise.all([
          api.get('/api/subscriptions/plans?audience=MATCHMAKER'),
  ...
  return (
    <div style={{ background: 'var(--bg-cream)', minHeight: '100vh', paddingBottom: 80 }}>
      ...
      {/* Hero — changes based on whether user has a paid plan */}
      {mySubscription && mySubscription.plan.value !== 'FREE' ? (
        /* ── Active paid plan hero ── */
        <div style={{
          background: `linear-gradient(135deg,${mySubscription.plan.color}12,${mySubscription.plan.color}06)`,
          borderBottom: `2px solid ${mySubscription.plan.color}25`,
          padding: '48px 20px 40px', textAlign: 'center',
        }}>
          <div style={{ fontSize: '3rem', marginBottom: 12 }}>
            {mySubscription.plan.value === 'GOLD' ? '✨' : mySubscription.plan.value === 'DIAMOND' ? '💎' : '👑'}
          </div>
          <h1 style={{ fontFamily: "'Playfair Display',serif", fontWeight: 800, fontSize: 'clamp(1.6rem,4vw,2.4rem)', color: mySubscription.plan.color, marginBottom: 8 }}>
            You're on the {mySubscription.plan.label} Plan
          </h1>
          <p style={{ fontFamily: "'Outfit',sans-serif", color: 'var(--text-mid)', fontSize: '0.97rem', maxWidth: 480, margin: '0 auto 20px', lineHeight: 1.6 }}>
            Your premium matchmaker benefits are active. View your plan details below or switch to a different plan.
          </p>
          {/* Expiry pill */}
          <div style={{
            display: 'inline-flex', alignItems: 'center', gap: 8, background: '#fff',
            border: `2px solid ${mySubscription.plan.color}40`, borderRadius: 50,
            padding: '8px 22px', fontFamily: "'Outfit',sans-serif",
            fontWeight: 600, color: mySubscription.plan.color, fontSize: '0.88rem',
          }}>
            ✅ Active until{' '}
            {new Date(mySubscription.expiresAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}
          </div>
          {/* Days left */}
          {(() => {
            const days = Math.ceil((new Date(mySubscription.expiresAt).getTime() - Date.now()) / 86400000);
            return (
              <div style={{ marginTop: 10, fontFamily: "'Outfit',sans-serif", fontSize: '0.82rem', color: days <= 14 ? '#E8735A' : 'var(--text-muted)' }}>
                {days <= 14 ? `⚠️ ${days} day${days !== 1 ? 's' : ''} remaining — consider renewing` : `${days} days remaining`}
              </div>
            );
          })()}
        </div>
        ...
        /* ── Free plan hero ── */
        <div style={{
          background: 'linear-gradient(135deg,#FFF0F8 0%,#FFF8F2 50%,#F8F0FF 100%)',
          ...
          <h1 style={{ fontFamily: "'Playfair Display',serif", fontWeight: 800, fontSize: 'clamp(1.8rem,4vw,2.8rem)', color: 'var(--text-dark)', marginBottom: 12 }}>
            Grow Your Matchmaking Business
          </h1>
          <p style={{ fontFamily: "'Outfit',sans-serif", color: 'var(--text-mid)', fontSize: '1.05rem', maxWidth: 560, margin: '0 auto 0', lineHeight: 1.65 }}>
            Unlock advanced tools to manage more client profiles, access detailed analytics, and build trust with the Verified Matchmaker badge.
          </p>
        </div>
      )}
        ...
        {DURATION_OPTIONS.map(opt => (
          <button
            ...
            style={{
              ...
              background: duration === opt.months ? 'linear-gradient(135deg,#E85AA3,#7B8FE8)' : '#fff',
          ...
          >
            {opt.label}
            {opt.months === 12 && (
              <span style={{ marginLeft: 6, background: '#E85AA3', color: '#fff', borderRadius: 50, fontSize: '0.7rem', padding: '2px 7px' }}>
                Best Deal
              </span>
            )}
          </button>
      ...
      {/* Matchmaker feature highlight strip */}
      <div style={{ maxWidth: 900, margin: '24px auto 0', padding: '0 20px' }}>
        <div style={{
          ...
          {[
            { icon: '👥', label: 'Manage Client Profiles', desc: 'Up to Unlimited' },
            { icon: '📊', label: 'Analytics Dashboard',    desc: 'Track success rates' },
            { icon: '✅', label: 'Verified Badge',         desc: 'Build client trust' },
            { icon: '🚀', label: 'Priority Listing',       desc: 'Appear at the top' },
          ].map(item => (
            <div key={item.label} style={{ textAlign: 'center' }}>
              <div style={{ fontSize: '1.6rem', marginBottom: 4 }}>{item.icon}</div>
              <div style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 700, fontSize: '0.85rem', color: '#2A1A1A' }}>{item.label}</div>
              <div style={{ fontFamily: "'Outfit',sans-serif", fontSize: '0.75rem', color: '#9A8A7A' }}>{item.desc}</div>
            </div>
          ))}
        </div>
      </div>

      {/* Section label when already subscribed */}
      {mySubscription && mySubscription.plan.value !== 'FREE' && (
        <div style={{ maxWidth: 1100, margin: '32px auto 0', padding: '0 20px' }}>
          <p style={{ fontFamily: "'Outfit',sans-serif", fontSize: '0.88rem', color: 'var(--text-muted)', textAlign: 'center' }}>
            Your current plan is highlighted below. You can switch to any other plan at any time.
          </p>
        </div>
      )}
      ...
      {/* Plan cards */}
      <div style={{
        maxWidth: 1100, margin: '16px auto 0', padding: '0 20px',
        ...
        {plans.map(plan => {
          ...
          const isFeatured = !isCurrent && plan.value === 'DIAMOND';
          const planIcon   = MM_PLAN_ICONS[plan.value] || '✨';
            ...
            <div
              ...
              style={{
                ...
                border: isCurrent
                  ...
                  : isFeatured ? `2px solid ${plan.color}` : '1.5px solid #F0E8E0',
                boxShadow: isCurrent
                  ? `0 0 0 4px ${plan.color}20, 0 12px 40px ${plan.color}25`
                  : isFeatured ? `0 8px 32px ${plan.color}22` : '0 2px 12px rgba(0,0,0,0.06)',
              ...
              onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.transform = ''; el.style.boxShadow = isCurrent ? `0 0 0 4px ${plan.color}20, 0 12px 40px ${plan.color}25` : isFeatured ? `0 8px 32px ${plan.color}22` : '0 2px 12px rgba(0,0,0,0.06)'; }}
            >
              {/* Current plan ribbon */}
              {isCurrent && (
                <div style={{
                  background: plan.color, color: '#fff', fontFamily: "'Outfit',sans-serif",
                  fontWeight: 700, fontSize: '0.72rem', textAlign: 'center',
                  padding: '5px 0', letterSpacing: '0.06em', textTransform: 'uppercase',
                }}>
                  ✓ Your Current Plan
                </div>
              )}
              ...
              <div style={{ padding: '16px 24px 20px' }}>
                <ul style={{ listStyle: 'none', margin: 0, padding: 0 }}>
                  {plan.features.slice(0, 8).map(a => (
                    <li key={a.feature.key} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '6px 0', borderBottom: '1px solid #FAF5F0', fontFamily: "'Outfit',sans-serif", fontSize: '0.84rem' }}>
                      <span style={{ fontSize: '0.95rem', flexShrink: 0, marginTop: 1 }}>{a.feature.icon || (a.included ? '✅' : '❌')}</span>
                      <div>
                ...
                <div style={{ marginTop: 20 }}>
                  {isCurrent ? (
                    <div style={{
                      textAlign: 'center', padding: '10px', borderRadius: 50,
                      background: `${plan.color}14`, border: `1.5px solid ${plan.color}30`,
                      fontFamily: "'Outfit',sans-serif", fontWeight: 700,
                      color: plan.color, fontSize: '0.88rem',
                    }}>
                      ✓ Active Plan
                    </div>
                  ) : plan.value === 'FREE' ? (
      ...
      {plans.length > 0 && allFeatureKeys.length > 0 && (
        <div style={{ maxWidth: 1100, margin: '48px auto 0', padding: '0 20px' }}>
          ...
          <p style={{ textAlign: 'center', color: 'var(--text-muted)', fontFamily: "'Outfit',sans-serif", marginBottom: 28, fontSize: '0.9rem' }}>
            Everything you need to run a professional matchmaking business
          </p>
          <div style={{ overflowX: 'auto', borderRadius: 20, boxShadow: '0 4px 20px rgba(0,0,0,0.06)', background: '#fff' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: "'Outfit',sans-serif" }}>
              <thead>
                <tr style={{ background: 'linear-gradient(135deg,#FFF0F8,#F0F0FF)' }}>
              ...
              <tbody>
                {allFeatureKeys.map((key, i) => {
                  ...
                  return (
                    <tr key={key} style={{ background: i % 2 === 0 ? '#FFFBF7' : '#fff' }}>
                      <td style={{ padding: '12px 20px', fontWeight: 500, fontSize: '0.86rem', color: 'var(--text-dark)', borderBottom: '1px solid #F5EDE6' }}>
                        <span style={{ marginRight: 8 }}>{feat?.icon}</span>
                        {feat?.title}
                      </td>
                      {plans.map(p => {
                        const a = p.features.find(fa => fa.feature.key === key);
                        return (
                          <td key={p.id} style={{ padding: '12px 20px', textAlign: 'center', borderBottom: '1px solid #F5EDE6' }}>
                            {!a ? (
                              <span style={{ color: '#D0C8C0', fontSize: '0.8rem' }}>—</span>
                            ) : a.included ? (
                              <div>
                                <span style={{ color: '#4ABEAA', fontSize: '1rem' }}>✓</span>
                                {a.quantity && (
                                  <div style={{ fontSize: '0.72rem', color: p.color, fontWeight: 600, marginTop: 2 }}>{a.quantity}</div>
                                )}
                              </div>
                            ) : (
                              <span style={{ color: '#E8D8D0', fontSize: '0.85rem' }}>✕</span>
                            )}
                          </td>
                        );
                      })}
      ...
      <div style={{ maxWidth: 700, margin: '60px auto 0', padding: '0 20px' }}>
        <h2 style={{ fontFamily: "'Playfair Display',serif", fontWeight: 800, fontSize: '1.5rem', color: 'var(--text-dark)', textAlign: 'center', marginBottom: 24 }}>
          Frequently Asked Questions
        </h2>
        {[
          { q: 'Can I upgrade my plan anytime?',
            a: 'Yes — upgrade at any time and your remaining days will be credited.' },
          ...
          { q: 'Do I need to upload a receipt?',
            a: 'Uploading a receipt speeds up verification significantly. We recommend attaching your bank transfer screenshot.' },
          { q: 'Will my profile stay visible if my plan expires?',
            a: 'Yes — you revert to Free-plan features after expiry. Your managed profiles remain but with lower limits.' },
        ].map((faq, i) => (
          <div key={i} style={{
            background: '#fff', borderRadius: 16, padding: '18px 22px',
            marginBottom: 12, border: '1px solid #F0E8E0',
            boxShadow: '0 2px 8px rgba(0,0,0,0.04)',
          }}>
            <div style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 700, fontSize: '0.95rem', color: 'var(--text-dark)', marginBottom: 6 }}>
              {faq.q}
            </div>
            <div style={{ fontFamily: "'Outfit',sans-serif", fontSize: '0.87rem', color: 'var(--text-mid)', lineHeight: 1.6 }}>
              {faq.a}
            </div>
          </div>
        ))}
